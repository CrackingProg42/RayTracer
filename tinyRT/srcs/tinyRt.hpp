...
public:
    ...
    Vector3 operator/(double b) const { return Vector3(x/b, y/b, z/b); }
    Vector3 operator*(const Vector3 &b) const { return Vector3(x*b.x, y*b.y, z*b.z); }
    Vector3& norm() { return *this = *this / length(); }
    double length() { return std::sqrt(x*x+y*y+z*z); }
    double dot(const Vector3 &b) const { return x*b.x+y*b.y+z*b.z; }
    Vector3 operator%(Vector3 &b) const { return Vector3(y*b.z-z*b.y, z*b.x-x*b.z, x*b.y-y*b.x); }
};
...
public:
    Vector3 o, d;
    Ray(Vector3 o0 = Vector3(), Vector3 d0 = Vector3()) : o(o0), d(d0) {}
};
...
public:
    ...
    double emission;
    int type; //
    void setMat(Vector3 color_ = Vector3(), double emission_ = 0, int type_=0) { color = color_; emission = emission_; type = type_; }
...
public:
	...
	Sphere(double r_ = 0, Vector3 c_ = 0) : c(c_), r(r_) {}
	...
	Vector3 normal(const Vector3& p0) const {
		return (p0 - c).norm();
	}
};
...
public:
	Vector3 n;
	double d;
	Plane(double d_ = 0, Vector3 n_ = 0) : d(d_), n(n_) {}
	double intersect(const Ray& ray) const {
	...
	Vector3 normal(const Vector3& p0) const { return n; }
};

class Intersection {
public:
	Intersection() : t(inf), obj(nullptr) {}
	Intersection(double t_, Object* obj_) : t(t_), obj(obj_) {}
