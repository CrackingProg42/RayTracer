...
public:
	...
	DEVICE Vector3 normal(const Vector3& p0) const {
		return (p0 - c).norm();
	}
};
...
public:
	...
	DEVICE Vector3 normal(const Vector3& p0) const { return n; }
};
...
GLOBAL void trace(Ray &ray, const Scene *scene, int depth, Vector3& clr, pl& params);
GLOBAL void calc_render(pl params, Scene **scene, Vector3 ***pix);
